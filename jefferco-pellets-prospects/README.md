...
Ce projet est **totalement indépendant** du reste du dépôt (site ENMA
Formation) : son propre `package.json`, ses propres dépendances, son propre
dossier `app/`. Aucun fichier hors de `jefferco-pellets-prospects/` n'est
utilisé ni modifié.

## Démarrer en local

```bash
cd jefferco-pellets-prospects
npm install
...
Pas de dépôt GitHub dédié pour l'instant (la création automatique d'un
nouveau dépôt a été refusée par les permissions de l'intégration GitHub
utilisée dans cette session — 403 « Resource not accessible by
integration »). Ce projet vit donc, pour l'instant, comme un dossier
totalement autonome dans le dépôt `Druelle2U0I/Fjhj`, sur la branche
`claude/pellets-prospect-app-3ywo73`.

Pour un vrai déploiement séparé : créer un dépôt GitHub vide (ex.
`jefferco-pellets-prospects`) et y pousser uniquement ce dossier, ou
configurer Vercel avec ce dossier comme « Root Directory » de ce dépôt.
Variables d'environnement nécessaires en production : `PELLETS_ADMIN_PASSWORD`,
`GITHUB_TOKEN`, `GITHUB_REPOSITORY`, `GITHUB_BRANCH`, `GITHUB_BASE_PATH`.
